...
bool loop() {
	...
	static double rot = 0;
	static std::vector<int> ballx;
	static std::vector<float> bally;
	static std::vector<float> ballyvel;
	...
	int dT = SDL_GetTicks() - ticks;
	ticks = SDL_GetTicks();
	for (int i = 0; i < ballx.size(); i++)
	{
		ballyvel[i] += (dT / 1000.0f) * GRAVITY;
		bally[i] += ballyvel[i] * (dT / 1000.0f);
		if (bally[i] > 448)
			bally[i] = 448;
	}
	...
	while ( SDL_PollEvent( &e ) != 0 ) {
		switch ( e.type ) {
			...
			case SDL_MOUSEBUTTONDOWN:
				ballx.push_back(e.button.x);
				bally.push_back(e.button.y);
				ballyvel.push_back(0);
				break;
		}
	}
...
bool init() {
	...
	buffer = SDL_LoadBMP("ball.bmp");
	ball = SDL_CreateTextureFromSurface(renderer, buffer);
	SDL_FreeSurface(buffer);
